...
@Component({
  ...
  template: `
    <mat-card>
      <mat-card-header class="bg-primary-theme text-white"><mat-card-title>Nuevo Rol</mat-card-title></mat-card-header>
        <mat-card-content>
          <form (submit)="this.createRol()">
            <div fxLayout="row wrap" [formGroup]="this.form">
              <div fxFlex="100">
                <mat-form-field>
                  <input matInput placeholder="Nombre del Rol" formControlName="nombre" maxlength="50">
            ...
            <mat-card-actions fxLayoutAlign="end center" fxLayoutGap="5px" >
              ...
              <button mat-flat-button color="accent" type="button" [disabled]="this.load" (click)="this.createRol()">Guardar</button>
            </mat-card-actions>
          </form>
        </mat-card-content>
    </mat-card>
...
export class AddRoleDialogComponent extends ClicComponent implements OnInit {
  public form: FormGroup;
  public confirm: boolean;
  public load: boolean;
  public error: string;
  constructor(private accessService: AccessService, private dialogRef: MatDialogRef<AddRoleDialogComponent>,
  ...
  createRol() {
    this.error = null;
    if (this.form.valid) {
      const role: AuthRole = this.form.value;
      this.load = true;
      this.accessService.requestRoleStore(role).subscribe(response => {
