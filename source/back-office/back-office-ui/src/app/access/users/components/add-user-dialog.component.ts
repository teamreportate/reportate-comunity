          ...
          <div fxLayout="row wrap" [formGroup]="this.form">
            <div fxFlex="100">
              <mat-form-field>
                  ...
                  <mat-error *ngIf="this.form.get('username').errors?.pattern && this.form.get('username').touched" class="text-danger font-11">Sólo se permiten caracteres alfanuméricos y guión bajo (_).</mat-error>
                  ...
                  <mat-error *ngIf="this.form.get('username').hasError('maxlength') && this.form.get('username').touched" class="text-danger font-11">Sólo se permite <strong>50 caracteres</strong>.</mat-error>
            ...
            <ng-container *ngIf="form.controls['authType'] && form.controls['authType'].value === 'PWD'">
              <div fxFlex="100">
                <mat-form-field>
                  <input matInput type="password" autocomplete="false" placeholder="Contraseña de Usuario" formControlName="password" [type]="ocultar ? 'password' : 'text'" (input)="onPasswordInput()" id="password" maxlength="20" >
              ...
              <div fxFlex="100">
                <mat-form-field>
                  <input matInput type="password" placeholder="Confirmar Contraseña" autocomplete="false" formControlName="passwordConfirm" [type]="ocultar ? 'password' : 'text'" (input)="onPasswordInput()" maxlength="20">
                  <button mat-icon-button color="accent" matSuffix (click)="ocultar = !ocultar" [attr.aria-pressed]="ocultar">
                    <mat-icon>{{ocultar ? 'visibility_off' : 'visibility'}}</mat-icon>
                  </button>
...
export class AddUserDialogComponent extends ClicComponent implements OnInit {
  ...
  constructor(private accessService: AccessService, private dialogRef: MatDialogRef<AddUserDialogComponent>, @Inject(MAT_DIALOG_DATA) public data: any,
  ...
  ngOnInit() {
    ...
    this.form = new FormGroup({
      ...
      username: new FormControl(null, Validators.compose([Validators.required, Validators.pattern('^[a-zA-Z0-9_]*$'), Validators.minLength(5), Validators.maxLength(50)])),
