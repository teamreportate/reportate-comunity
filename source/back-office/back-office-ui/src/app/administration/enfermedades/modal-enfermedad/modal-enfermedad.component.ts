...
@Component({
  ...
  styleUrls: ['./modal-enfermedad.component.sass'],
})
export class ModalEnfermedadComponent extends ClicComponent implements OnInit {
  ...
  constructor(private service: AdministrationService, private builder: FormBuilder,
    private dialogRef: MatDialogRef<ModalEnfermedadComponent>,
  ...
  ngOnInit() {
    this.form = this.builder.group({
      ...
      // tslint:disable-next-line:max-line-length
      mensajeDiagnostico: new FormControl(this.data.mensajeDiagnostico, Validators.compose([Validators.maxLength(4000)])),
      enfermedadBase: new FormControl(this.data.enfermedadBase),
    });
    if (this.operacion === 'CRE') {
      this.form.get('enfermedadBase').setValue(false);
    }
  }
  close(): void {
  ...
  save() {
    if (isNaN(this.data.id)) {
      ...
      this.service.requestSaveEnfermedad(this.form.value).subscribe(response => {
        this.blockUi.stop();
        const msg = { title: 'Registro de Enfermedad', detail: 'Enfermedad registrada satisfactoriamente.'};
      ...
      this.blockUi.start('Procesando solicitud...');
      this.service.requestUpdateEnfermedad(this.data.id, this.form.value).subscribe(response => {
        this.blockUi.stop();
        const msg = { title: 'Actualizar Enfermedad', detail: 'Enfermedad actualizada satisfactoriamente.'};
        this.dialogRef.close(msg);
      }, error => {
        this.blockUi.stop();
        if (error) { this.notifierError(error); }
